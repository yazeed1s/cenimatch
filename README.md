...
| **llm client**      | `internal/llm/openrouter.go`             | polling client with multi-model fallback (Llama 3.3, Hermes, Qwen). safe SQL generation via `query_guard.go`. `SchemaPrompt` for context injection.                                                                                                                                              |
| **chat service**    | `internal/service/chat.go`               | handles natural language to SQL translation flow. coordinates LLM client and query guard.                                                                                                                                                                                                       |
| **chat frontend**   | `ui/src/pages/ChatPage.tsx`              | full chat interface with message history, dynamic result rendering (movie grids or tables), and SQL transparency toggle.                                                                                                                                                                        |
| **graph recs**      | `internal/infra/repository/movies_repo.go`| Apache AGE traversal queries for personalized "discovered via graph" recommendations based on user history and ratings.                                                                                                                                                                         |
